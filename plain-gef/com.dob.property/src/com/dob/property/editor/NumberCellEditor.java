 ...
 *
 */
public class NumberCellEditor extends ObjectCellEditor
	...
	protected static final MinmaxValidator[] sMinMaxValidators = { null,
			new MinmaxValidator(MIN_BYTE, MAX_BYTE), new MinmaxValidator(MIN_DOUBLE, MAX_DOUBLE),
			new MinmaxValidator(MIN_FLOAT, MAX_FLOAT),
			new MinmaxValidator(MIN_INTEGER, MAX_INTEGER), new MinmaxValidator(MIN_LONG, MAX_LONG),
	...
	static {
		...
		sNotIntegerError = "The value is not an integer.";
		sMinValue = "Min";
		sMaxValue = "Max";
	}
	...
	protected String isCorrectObject(Object value) {
		return (value == null || value instanceof Number) ? null
				: (fFormatter.isParseIntegerOnly() ? sNotIntegerError : sNotNumberError);
	}
	...
	protected String isCorrectString(String value) {
		...
		if ((fNumberType == DOUBLE || fNumberType == FLOAT)
			...
			if (fNumberType == LONG && result instanceof Double) {
				result = (result.doubleValue() < 0) ? MinmaxValidator.LONG_UNDERFLOW
